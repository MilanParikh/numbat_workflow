...
workflow numbat_workflow {
  ...
  if(ran_topmed) {
      call create_allele_file {
        input:
            output_directory = sub(output_directory, "/+$", ""),
            topmed_vcf_folder = topmed_vcf_folder,
            label = label,
            sample_name = sample_name,
            preemptible = preemptible,
            disk_space = disk_space,
  ...
  call run_numbat {
    input:
      label = label,
      output_directory = sub(output_directory, "/+$", ""),
      allele_counts = select_first([create_allele_file.allele_counts, pileup_and_phase.allele_counts]),
...
task create_allele_file {
  ...
  command <<<
    ...
    arg_vals <- paste(args, collapse = ' ')
    print(glue("Rscript /numbat/inst/bin/create_allele_file.R {arg_vals}"))

    system(paste0(glue("Rscript /numbat/inst/bin/create_allele_file.R {arg_vals}")))
